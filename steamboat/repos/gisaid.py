...
def gisaid_formatter(
    metadata: dict, fasta: str, sequencer: str, constants: dict
) -> dict:
    """
    ...
    authors = "{}, and {}".format(
        ", ".join(constants["AUTHORS"][:-1]), constants["AUTHORS"][-1]
    )
    ...
    location = (
        f'{constants["CONTINENT"]} / {constants["COUNTRY"]} / {constants["STATE"]}'
    )
    return OrderedDict(
        (
            ...
            (
                "covv_sampling_strategy",
                constants["GISAID_SAMPLING_STRATEGY"]
                if "GISAID_SAMPLING_STRATEGY" in constants
                else "",
            ),
            ("covv_gender", metadata["sex"].lower()),
            (
                "covv_patient_age",
                ">90" if int(metadata["age"]) >= 90 else metadata["age"],
            ),
...
def parse_consensus_assemblies(assemblies: str, extension: str) -> dict:
    ...
    for assembly in Path(assemblies).glob(f"*.{extension}"):
        ...
        if len(fasta) == 1:
            ...
            logging.error(
                f"Consensus assembly for {assembly} contains more than one sequence"
            )
        else:
